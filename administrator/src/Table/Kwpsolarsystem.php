...
namespace Joomla\Component\Kwpsolarsystem\Administrator\Table;
...
/**
 * Kwpsolarsystem table
 */
 
class KwpsolarsystemTable extends Table
{
    /**
    ...
    {
        parent::__construct('#__kwpsolarsystem', 'id', $db);
    }
    ...
    public function bind($data, $ignore = '')
    {
        ...
        if(($task == 'save' || $task == 'apply') && (!Factory::getUser()->authorise('core.edit.state','com_kwpsolarsystem') && !isset($data['id']))) {
            $data['state'] = 0;
        }
     ...
     /*
        ...
        if(!Factory::getUser()->authorise('core.admin', 'com_kwpsolarsystem.kwpsolarsystem.'.$data['id']))
        {
            $actions = ContentHelper::getActions('com_kwpsolarsystem','kwpsolarsystem');
            $defaultActions = Access::getAssetRules('com_kwpsolarsystem.kwpsolarsystem.'.$data['id'])->getData();
            $jaccessRules = [];
    ...
    {
        $k = $this->_tbl_key;
        return 'com_kwpsolarsystem.kwpsolarsystem.' . (int) $this->$k;
    }
    ...
    protected function _getAssetParentId(Table $table = NULL, $id = NULL) : int
    {
        ...
        $assetParent->loadByName('com_kwpsolarsystem');
